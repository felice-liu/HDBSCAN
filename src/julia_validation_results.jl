...
function load_dataset(dataset_name::String)
    ...
    elseif cfg[:kind] == :real
        df = CSV.read(path, DataFrame)

        # keep only the feature columns
        Xdf = select(df, cfg[:feature_cols])

        # median-impute missing values and coerce to Float64
        fill_missing_median!(Xdf)

        return Matrix{Float64}(Xdf)
...
function run_one(dataset_name::String, hdb)
    ...
    labels = Int.(hdb.labels_)
    probabilities = Float64.(hdb.probabilities_)
